...
if name:
    ...
    if question_index == len(video_paths) - 1 and st.button("Submit Answers"):
        ...
        # Creiamo il dizionario per la riga dei dati
        new_data = {
            "Name": name,
            "Clinician": clinician,
        ...
        # Aggiungiamo le risposte alle domande
        for i in range(len(video_paths)):
            new_data[f"Question {i+1}"] = st.session_state["responses"][i] if st.session_state["responses"][i] else "No Response"

        df_new = pd.DataFrame([new_data])  # Creiamo il DataFrame da una lista di dizionari

        # Se il file esiste, lo carichiamo e aggiungiamo i nuovi dati
